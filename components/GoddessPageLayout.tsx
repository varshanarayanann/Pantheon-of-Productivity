...
const GoddessPageLayout: React.FC<GoddessPageLayoutProps> = ({ goddess, children }) => {
  ...
  return (
    <div className="animate-fadeIn">
      <div className="mb-8">
        <Link to="/" className="text-slate-500 hover:text-slate-800 transition-colors duration-200 group flex items-center">
      ...
      <header className="text-center mb-12">
        <div className={`inline-block p-4 ${bgColorClass} rounded-full mb-4`}>
            <div className={colorClass}>{React.cloneElement(goddess.icon, {className: 'w-16 h-16'})}</div>
        </div>
        <h1 className={`text-5xl font-serif font-bold ${colorClass}`}>{goddess.name}</h1>
        <p className="text-xl text-slate-600 mt-2">{goddess.title}</p>
        <p className="text-md text-slate-500 mt-4 max-w-2xl mx-auto">{goddess.description}</p>
      </header>

      <div className="bg-white rounded-2xl shadow-xl p-8 border border-slate-200">
        <h2 className="text-3xl font-bold font-serif text-slate-700 mb-6 text-center">{goddess.toolName}</h2>
        {children}
      </div>
    </div>
  );
