...
const AthenaPage: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      const newChat = ai.chats.create({
        ...
        config: {
          systemInstruction: 'You are Athena, the Greek goddess of wisdom, strategy, and crafts. Act as a wise and patient tutor. Your goal is to guide users to discover answers for themselves rather than giving direct answers. Ask probing questions and encourage critical thinking. Keep your responses concise and thoughtful.',
        },
      });
      setChat(newChat);
      setMessages([{
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(e);
      setMessages(prev => [...prev, {role: 'model', text: 'My apologies, a momentary disruption in the cosmos has occurred. Please check your connection or API key and try again.'}]);
    } finally {
      setIsLoading(false);
    }
  };
